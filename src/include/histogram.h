...
public:
	...
	void print_stats(void) {
		...
		INFO(NCCL_INIT, "histogram %s", description.c_str());
		INFO(NCCL_INIT, "%s  min: %ld, max: %ld, num_samples: %lu",description.c_str(),
		...
		for (size_t i = 0 ; i < bins.size() ; ++i) {
			...
			ss  << "    " << bins[i];
			INFO(NCCL_INIT, "%s %s", description.c_str(), ss.str().c_str());
		}
	}
